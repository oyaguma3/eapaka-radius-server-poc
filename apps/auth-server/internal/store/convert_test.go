...
func TestMapToStructMissingField(t *testing.T) {
	...
	if s.Age != 0 {
		t.Errorf("Age: got %v, want 0 (zero value)", s.Age)
	}
	if s.Active != false {
		t.Errorf("Active: got %v, want false (zero value)", s.Active)
	}
}
...
func TestMapToStructInvalidUint8(t *testing.T) {
	m := map[string]string{
		"level": "999",
	}
	var s testStruct
	err := MapToStruct(m, &s)
	if err == nil {
		t.Fatal("expected error for invalid uint8 value (overflow)")
	}
}
