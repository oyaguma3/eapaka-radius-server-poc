...
| ドキュメント | 内容 |
|-------------|------|
| D-04 ログ仕様設計書 (r18) | ログフォーマット、event_id体系、IMSIマスキング仕様、lnavフォーマット定義 |
| D-08 インフラ設定・運用設計書 (r13) | Fluent Bit設定（v4.2, YAML形式）、logrotate設定、ログ出力パス |
| B-02 アプリケーションデプロイ手順書 (r9) | lnavフォーマットファイル配置手順（§10）、トラブルシューティング（§13） |
...
│                 │ forward                                  │
│  ┌──────────────▼──────────────────────┐                  │
│  │  Fluent Bit v4.2 (タグ付け・分割)    │                  │
│  │  設定: fluent-bit.yaml (YAML形式)    │                  │
│  └──────────────┬──────────────────────┘                  │
└─────────────────┼─────────────────────────────────────────┘
                  │ ファイル出力
                  ▼
...
| `others.log` | ヘルスチェックログ（Vector API/Gatewayの `/health`） | 〜1MB |

> **注記:** `others.log` にはFluent Bitの `rewrite_tag` フィルタにより `healthcheck.*` タグへリタグされたヘルスチェックログのみが出力される。詳細はD-08 §3.2を参照。
...
| フィールド名 | 型 | 説明 | 例 |
|-------------|------|------|-----|
| `time` | String | タイムスタンプ (RFC3339Nano) | `"2026-02-23T14:23:40.210276945+09:00"` |
...
```json
{
  "time": "2026-02-22T10:00:00.123456789+09:00",
  "level": "INFO",
  "app": "auth-server",
...
```
2026-02-22T10:00:00.123456789+09:00 INFO auth-server AUTH_OK [440101********0] authentication success
```
...
| `ACCT_ON` | INFO | Accounting-On受信（NAS起動通知） | 情報 -- NAS起動を確認 |
| `ACCT_OFF` | INFO | Accounting-Off受信（NASシャットダウン通知） | 情報 -- NAS停止を確認 |
...
### 7.4 Accounting-On/Offフロー追跡

NASの起動/停止通知はセッション操作を伴わず、ログ出力のみ行われる。NAS-Identifier / NAS-IP-Address でNASを特定できる。

```sql
;SELECT log_time, event_id, nas_ip_address, nas_identifier
 FROM logline
 WHERE event_id IN ('ACCT_ON', 'ACCT_OFF')
 ORDER BY log_time
```
...
## 13. lnavフォーマットカスタマイズの注意点

本プロジェクトのlnavフォーマット定義（`eap_aka_log.json`）を運用・カスタマイズする際の注意点を記載する。これらはPR #8以降の調査で判明した知見である。

### 13.1 bunyan組み込みフォーマットとの競合

lnav v0.11.2 はフォーマット定義をアルファベット順にスキャンし、最初にマッチしたフォーマットを適用する。lnavに組み込まれている `bunyan_log` フォーマットは、JSONで5フィールド以上を持つログ行に広くマッチする。

カスタムフォーマットのフォーマット名（JSONのトップレベルキー）がアルファベット順で `bunyan_log` より後に位置する場合、bunyanが先にマッチしてカスタムフォーマットが適用されない。

**本プロジェクトの対策:** フォーマット名を `aka_radius_log`（`a` 始まり）とし、`bunyan_log`（`b` 始まり）より先にスキャンされるようにしている。

> **注意:** フォーマット名を変更する場合は、必ず `bunyan_log` よりアルファベット順で前になる名前を選択すること。

### 13.2 timestamp-format の %N 9桁固定問題

lnavの `timestamp-format` で使用される `%N` はナノ秒を表すが、**常に9桁固定**でマッチする。一方、Go `slog` の `time.RFC3339Nano` 形式は末尾のゼロを削除するため、ナノ秒部分は3桁〜9桁の可変長になる。

- 9桁の場合（例: `.356730959`）→ `%N` でマッチする
- 3桁の場合（例: `.393`）→ `%N` でマッチしない

**本プロジェクトの対策:** `timestamp-format` を定義から削除し、lnavのISO 8601自動検出に委任している。lnavはJSONフォーマットのタイムスタンプフィールドに対してISO 8601形式を自動認識でき、可変長のナノ秒にも正しく対応する。

### 13.3 file-pattern による対象ファイル限定

`file-pattern` を設定することで、フォーマットの適用対象ファイルを正規表現で限定できる。これにより以下のメリットがある。

- 無関係なログファイル（`valkey.log`, `others.log` 等）へのマッチング試行を回避
- bunyan等の他フォーマットとの競合リスクをさらに低減
...
新しいアプリケーションノードを追加した場合は、`file-pattern` にそのログファイル名パターンを追加すること。

### 13.4 sample定義の重要性

`sample` セクションにはフォーマットのテスト用サンプルログ行を定義する。lnav -C（構文検証）実行時にサンプル行が正しくパースできるかを自動検証するため、以下の効果がある。

- フォーマット定義の変更後、`lnav -C` で即座にパースエラーを検出できる
- 異なるナノ秒桁数（3桁、9桁）やログレベル（INFO, ERROR）のバリエーションを含めることで、実運用でのマッチング問題を事前に発見できる

**本プロジェクトでは3種のサンプルを定義:**

1. ナノ秒9桁 + INFOレベル（起動ログ）
2. ナノ秒9桁 + INFOレベル（リクエスト完了ログ、数値フィールドあり）
3. ナノ秒9桁 + ERRORレベル（認証失敗ログ、`level` 明示指定あり）
...
|------|------|------|
| r1 | 2026-02-22 | 初版作成 |
| r2 | 2026-02-23 | 関連ドキュメント参照版数を更新（B-02 r2→r4） |
| r3 | 2026-02-23 | Fluent Bit 4.2アップグレード＋YAML移行反映: §1.4 D-08参照版数更新（r10→r12）、§2.1ログ収集フロー図にFluent Bitバージョン・設定形式を追記、§4.5 SQLカラム一覧に`app`追加（lnavフォーマット修正に伴う） |
| r4 | 2026-02-28 | lnavフォーマットtimestamp-format修正対応: §3.1, §3.5のサンプルタイムスタンプを実運用形式（RFC3339Nano+数値TZオフセット）に更新、関連ドキュメント参照版数更新（D-04 r13→r16、B-02 r4→r8） |
| r5 | 2026-02-28 | §13新設: lnavフォーマットカスタマイズの注意点（bunyan競合・%N固定桁・file-pattern・sample定義）、旧§13改訂履歴を§14にリナンバリング、関連ドキュメント参照版数更新（D-04 r16→r17、B-02 r8→r9） |
| r6 | 2026-03-05 | §5.2にACCT_ON/ACCT_OFF追加、§7.4にAccounting-On/Offフロー追跡追加、§2.2 others.log説明をヘルスチェックログに更新・注記追加、§1.4参照版数更新（D-04 r17→r18、D-08 r12→r13） |
