...
| ツール | 用途 | パス |
|:------|:-----|:----|
| eapaka_test | EAP-AKA/AKA' フル認証テスト | `/home/sumitakekino/devtools/eapaka_test/eapaka_test` |
...
```bash
# eapaka_testのパス
EAPAKA_TEST="/home/sumitakekino/devtools/eapaka_test"
...
cd /home/sumitakekino/projects/eapaka-radius-server-poc/deployments
cp .env .env.bak.plmn
...
| ID | シナリオ名 | ツール | PASS条件 |
|:---|:---------|:------|:--------|
| INT-FAULT-001 | Vector API停止 | eapaka_test + docker compose | Reject → 復旧後Accept |
| INT-FAULT-002 | Valkey停止（認証） | eapaka_test + docker compose | 無応答（タイムアウト）→ 復旧後Accept（※1） |
| INT-FAULT-003 | Valkey停止（課金） | radclient + docker compose | 無応答（タイムアウト）→ 復旧後Response（※1） |
| INT-FAULT-004 | Vector Gateway停止 | eapaka_test + docker compose | Reject → 復旧後Accept |
| INT-FAULT-005 | Circuit Breaker動作 | eapaka_test + docker compose | CB Open/Half-Open/Closed遷移ログ確認 |
...
> **※1 Valkey停止時の挙動（r3追記）**: Valkey停止中はRADIUSクライアントのSecret検索が不可能なため、サーバーはReject/Responseパケットを構築できず**無応答（タイムアウト）**となる。これはRejectよりも安全な障害動作であり、PASS判定とする。復旧後にテストデータの再投入（`FLUSHDB`されている場合）が必要な点にも注意。
...
# ログ確認
docker compose logs auth-server --since=5m | grep -E "VECTOR_API_ERR|VALKEY_CONN_ERR|CB_"
```
...
9. **Valkey停止テスト後のデータ再投入（r3追記）**: INT-FAULT-002/003でValkeyを停止・再起動した際、Valkeyがメモリ上のデータを保持している場合と`FLUSHDB`相当の状態になる場合がある。復旧後の認証テスト（Accept確認）の前に、テストデータ（加入者・クライアント・ポリシー）が残存しているか確認し、必要に応じて再投入すること。
...
| 39 | INT-FAULT-002 | Valkey停止（認証） | G9 | eapaka_test + docker | Reject→復旧Accept |
| 40 | INT-FAULT-003 | Valkey停止（課金） | G9 | radclient + docker | Response→復旧正常 |
...
> **テストケースファイルのベースパス**: `/home/sumitakekino/devtools/eapaka_test/testdata/cases/`
> **eapaka_test設定ファイル（デフォルト）**: `/home/sumitakekino/devtools/eapaka_test/configs/config_testvector.yaml`
> **eapaka_test設定ファイル（IMSI 003用）**: `/home/sumitakekino/devtools/eapaka_test/configs/config_testvector_imsi003.yaml`
