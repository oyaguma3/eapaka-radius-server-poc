# EAP-AKA RADIUS PoC環境 ドキュメント一覧 (r19)
...
| D-08 | インフラ設定・運用設計書 | r11 | 2026-02-23 | Docker Compose設定、Valkey設定、Fluent Bit設定（fluent/fluent-bit統一）、UFW設定、運用手順、IMSIマスキング環境変数（4コンポーネント限定）、Valkeyバージョン9.0、ヘルスチェック方針（curl -fsS）、テストベクターモード環境変数、B-02スコープ修正（B-01境界整合） |
...
| O-05 | ログ解析ガイド | **完了 (r2)** | D-04の完成後 | lnavの使い方、頻出クエリ集、障害調査パターン |
