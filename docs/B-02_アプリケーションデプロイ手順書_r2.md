...
| D-04 ログ仕様設計書 (r12) | lnavフォーマット定義 |
| D-08 インフラ設定・運用設計書 (r9) | docker-compose.yml、.env、運用手順、バックアップ、logrotate |
...
Admin TUIが正常に起動し、Valkeyに接続できることを確認する。
...
| 設定ファイルエラー | `configs/fluent-bit/fluent-bit.conf` の内容を確認 |
| ディレクトリ権限不足 | `chmod 755 ~/eapaka-radius-server-poc/deployments/logs_on_host/` |

### 12.4 Admin TUIが接続できない場合
