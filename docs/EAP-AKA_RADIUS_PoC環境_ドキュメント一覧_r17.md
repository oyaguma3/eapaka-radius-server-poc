# EAP-AKA RADIUS PoC環境 ドキュメント一覧 (r17)
...
| D-04 | ログ仕様設計書 | r13 | 2026-02-18 | ログフォーマット、event_id定義、Vector Gateway対応、EAP_INVALID_STATE追加、IMSIマスキング（4コンポーネント対応、Admin TUI除外明記）、Acct Server PKT_RECV追加、SQN_CONFLICT_ERR追加 |
...
| D-08 | インフラ設定・運用設計書 | r10 | 2026-02-18 | Docker Compose設定、Valkey設定、Fluent Bit設定（fluent/fluent-bit統一）、UFW設定、運用手順、IMSIマスキング環境変数（4コンポーネント限定）、Valkeyバージョン9.0、ヘルスチェック方針（curl -fsS）、テストベクターモード環境変数、B-02スコープ修正（B-01境界整合） |
...
| T-03 | 結合テスト仕様書 | r3 | 2026-02-16 | コンポーネント間連携テスト、シナリオテスト、テストベクターモード検証、Valkeyデータ整合性検証、Secret体系明確化、SQN再同期手順改訂、IMSI 003専用config追加、障害系PASS条件修正 |
...
| B-02 | アプリケーションデプロイ手順書 | r2 | 2026-02-16 | リポジトリクローン、.env作成、Docker Compose起動、Admin TUI配置、logrotate設定、バックアップスクリプト配置、lnavフォーマット配置 |
...
| O-05 | ログ解析ガイド | **完了 (r1)** | D-04の完成後 | lnavの使い方、頻出クエリ集、障害調査パターン |
