...
| ツール | 用途 | パス |
|:------|:-----|:----|
| eapaka_test | EAP-AKA/AKA' フル認証テスト | 開発環境に配置（S-01 セクション2参照） |
...
```bash
# eapaka_testバイナリのパス（開発環境に配置、S-01 セクション2参照）
EAPAKA_TEST="/path/to/devtools/eapaka_test"
# 設定ファイル・テストケースはsupplement配下を参照
...
# INT-POLICY-006: ワイルドカードSSID（IMSI 006使用）
# 注意: eapaka_testはidentityオーバーライド時もconfigのsqn_initial_hexを初期SQNとして
# 使用するため、サーバー側SQNをsqn_initial_hexと同等の値に設定する必要がある。
# config_testvector.yaml の sqn_initial_hex が "FF9BB4D0B607" の場合:
...
**目的**: Accounting Start/Interim/Stop の処理、On/Off（NAS起動/シャットダウン通知）の受容、重複検出、不明Status-Typeの動作を検証する。
...
| INT-ACCT-ON-017 | Accounting-On（NAS起動通知） | radclient | Accounting-Response, `ACCT_ON` ログ（`nas_identifier` 含む） |
| INT-ACCT-OFF-018 | Accounting-Off（NASシャットダウン通知） | radclient | Accounting-Response, `ACCT_OFF` ログ（`nas_identifier` 含む） |
...
echo "Acct-Status-Type=99
Acct-Session-Id=unknown-test
NAS-IP-Address=127.0.0.1" | \
radclient -x localhost:1813 acct TESTSECRET123
# 期待: タイムアウト（無応答）

# Step 7: Accounting-On（INT-ACCT-ON-017）
echo "Acct-Status-Type=Accounting-On
NAS-IP-Address=127.0.0.1
NAS-Identifier=Customer01" | \
radclient -x localhost:1813 acct TESTSECRET123
# 期待: Accounting-Response

# Step 8: Accounting-Off（INT-ACCT-OFF-018）
...
# Step 10: ログ確認
docker compose logs acct-server --since=5m | grep -E "ACCT_START|ACCT_INTERIM|ACCT_STOP|ACCT_DUPLICATE|ACCT_ON|ACCT_OFF"
```
...
7. **SQN初期値とリセット運用（r3追記、r5補足）**: テストベクターモードではSQN increment step = 0x20（32進）であり、テストを繰り返すとSQNが急速に増加する。高SQN値（例: `FF9BB4D0B607`）ではVector APIの「SQN difference exceeds allowed range」チェックに抵触し認証が失敗する場合がある。**SQNリセット時は、eapaka_test設定ファイルの`sqn_initial_hex`の値に応じた適切な値を設定すること。**
   - **configのプライマリIMSI（例: IMSI 000）**: テストケースの`identity`でオーバーライドされないIMSI。SQNストアファイルで直接管理されるため、低値（`000000000001`）へのリセットでよい。
   - **identityオーバーライドIMSI（例: IMSI 001, 002, 006, 007）**: eapaka_testはSQNストアに未登録のIMSIに対して`sqn_initial_hex`（例: `FF9BB4D0B607`）を初期SQNとして使用する。そのため、**サーバー側SQNも`sqn_initial_hex`と同等の値に設定する必要がある**。低値にリセットするとクライアント側SQNとの差が大きくなり、「SQN difference exceeds allowed range」エラーが発生する。
   ```bash
   # SQNリセット例（全テストベクターIMSI）
   # プライマリIMSI（IMSI 000）: 低値でOK
   docker compose exec valkey redis-cli -a "$VALKEY_PASSWORD" HSET sub:001010000000000 sqn "000000000001"
   # identityオーバーライドIMSI: sqn_initial_hex に合わせる
   ...
   # IMSI 003はconfig_testvector_imsi003.yamlのsqn_initial_hexに合わせること
...
13. **依存ライブラリ更新時のDockerイメージ再ビルド（r5追記）**: go-eapaka等の外部パッケージを更新した場合、`go.mod`の変更だけではDocker コンテナに反映されない。**必ず`docker compose build <service>`でイメージを再ビルドし、`docker compose up -d`でコンテナを再作成すること。** 古いイメージのまま実行すると、サーバーとテストツール間で暗号処理（PRF、鍵導出等）の不一致が発生し、AT_MAC検証失敗やAuthentication-Rejectとなる。
    ```bash
    # 例: auth-server の依存ライブラリ更新後
...
14. **eapaka_test SQNストアファイルのIMSI間干渉（r4追記）**: eapaka_testのSQNストアファイルは `sim.imsi` をキーにSQNを管理するが、設定ファイルのIMSIとテストケースのIMSIが異なる場合、設定ファイルのIMSI（デフォルト: `001010000000000`）のSQNが使用される。異なるIMSI（例: `001010000000007`）のテスト実行後にIMSI 000のサーバー側SQNとの不整合が発生する場合がある。テストケース切替時にはSQNストアファイルの削除を推奨する。
...
| 32 | INT-ACCT-ON-017 | Accounting-On（NAS起動通知） | G6 | radclient | Response + `ACCT_ON` ログ |
| 33 | INT-ACCT-OFF-018 | Accounting-Off（NASシャットダウン通知） | G6 | radclient | Response + `ACCT_OFF` ログ |
| 34 | INT-VALKEY-003 | 課金後ステートデータ | G6 | redis-cli | acct:seen状態確認 |
| 35 | INT-FLOW-001 | 認証→課金一連フロー | G7 | eapaka_test + radclient | 全ステップ成功 |
| 36 | INT-VALKEY-004 | TTLライフサイクル検証 | G7 | redis-cli | TTL値正常 |
| 37 | INT-VALKEY-006 | 総合整合性検証 | G7 | redis-cli | 全データ整合性OK |
| 38 | INT-GW-PLMN-009 | PLMN一致（内部API） | G8 | eapaka_test | Accept (BackendID 00) |
| 39 | INT-GW-PLMN-010 | 未実装Backend (PLMN 441999) | G8 | eapaka_test | Reject (501) |
| 40 | INT-FAULT-001 | Vector API停止 | G9 | eapaka_test + docker | Reject→復旧Accept |
| 41 | INT-FAULT-002 | Valkey停止（認証） | G9 | eapaka_test + docker | 無応答→復旧Accept |
| 42 | INT-FAULT-003 | Valkey停止（課金） | G9 | radclient + docker | 無応答→復旧Response |
| 43 | INT-FAULT-004 | Vector Gateway停止 | G9 | eapaka_test + docker | Reject→復旧Accept |
| 44 | INT-FAULT-005 | Circuit Breaker動作 | G9 | eapaka_test + docker | CB状態遷移確認 |
| 45 | INT-TIMEOUT-001 | 疑似タイムアウト | G9 | radclient + redis-cli | Reject |
| 46 | INT-TIMEOUT-002 | Valkey TTL短縮方式 | G9 | redis-cli | キー消滅確認 |
...
> **テストケースファイルのベースパス**: `docs/supplement/eapaka_test/testdata/cases/`（プロジェクトルート相対）
> **eapaka_test設定ファイル（デフォルト）**: `docs/supplement/eapaka_test/configs/config_testvector.yaml`
> **eapaka_test設定ファイル（IMSI 003用）**: `docs/supplement/eapaka_test/configs/config_testvector_imsi003.yaml`
> **eapaka_testバイナリ**: 開発環境に配置（S-01 セクション2参照）
...
| r5 | 2026-02-22 | go-eapaka PRF修正（FIPS 186-2準拠）後のEAP-AKA結合テスト再実行で得られた知見を反映: |
| | | - G5 INT-POLICY-006: SQNリセット値を`000000000001`から`FF9BB4D0B607`（sqn_initial_hex準拠）に修正 |
| | | - AI自動実行ガイド #7: SQN リセット運用を大幅補足 — identityオーバーライドIMSI（001, 002, 006, 007等）はeapaka_testが`sqn_initial_hex`を初期SQNとして使用するため、サーバー側SQNも同値に設定する必要がある旨を明記 |
| | | - AI自動実行ガイド #13（新規）: 依存ライブラリ（go-eapaka等）更新時のDockerイメージ再ビルド必須の注意事項を追加。古いイメージではPRF/鍵導出の不一致によりAT_MAC検証失敗が発生する |
| | | - AI自動実行ガイド 旧#13→#14: 番号繰り下げ |
| r6 | 2026-02-24 | eapaka_test利用ノウハウ補足資料（S-01）作成に伴うパス参照の一般化: |
| | | - 参照ドキュメントにS-01 eapaka_test利用ノウハウ(r1)を追加 |
| | | - eapaka_test設定ファイル・テストケースのパス参照を`docs/supplement/eapaka_test/`配下に変更 |
| | | - 付録Bのベースパス定義をsupplement配下に更新 |
| | | - セクション2.3のeapaka_testパスをS-01参照に変更 |
